...
class NCEFunction(Function):
    ...
    def forward(self, x, y, memory, idx, params):
        ...
        # inner product
        with torch.no_grad():
          xtemp = x.reshape(batchSize, inputSize, 1)
        #out = torch.bmm(weight, x.data.resize_(batchSize, inputSize, 1))
        out = torch.bmm(weight,xtemp)
        out.div_(T).exp_() # batchSize * self.K+1
        x.data.reshape(batchSize, inputSize)
        ...
        out.div_(Z).reshape(batchSize, K+1)
    ...
    def backward(self, gradOutput):
        ...
        gradOutput.data = gradOutput.data.reshape(batchSize, 1, K+1)
        # gradient of linear
        #print(weight.shape)
        weight = weight.cuda()
        ...
        memory = memory.cuda()
        y = y.cuda()
        updated_weight = updated_weight.cuda()
